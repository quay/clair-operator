...
pub async fn validate(client: &Client, cfg: &v1alpha1::ConfigSource) -> Result<Validate> {
    ...
    assert!(cfg
        .dropins
        .iter()
        .all(|c| c.secret_key_ref.is_some() || c.config_map_key_ref.is_some()));
    ...
    let mut order = cfg
        ...
        .enumerate()
        .map(|(i, v)| {
            let key = if let Some(cmref) = &v.config_map_key_ref.as_ref() {
                &cmref.key
            } else if let Some(secref) = &v.secret_key_ref.as_ref() {
                &secref.key
            } else {
                unreachable!()
            };
    ...
    for (i, _) in order {
        let r = &cfg.dropins[i];
        let src = if let Some(cmref) = &r.config_map_key_ref {
            Source::ConfigMap(cmref)
        } else if let Some(secref) = &r.secret_key_ref {
...
enum Source<'a> {
    ConfigMap(&'a v1alpha1::ConfigMapKeySelector),
    Secret(&'a v1alpha1::SecretKeySelector),
}
